...
#     print(f"♻️ Found existing weights file at '{MODEL_SAVE_PATH}'. Attempting state recovery...")
#     try:
#         checkpoint = torch.load(MODEL_SAVE_PATH, map_location=DEVICE)
        
#         # Pull model parameters safely
#         if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
#             model.load_state_dict(checkpoint['model_state_dict'])
#             print("   ✅ Loaded model parameter architecture weights.")
#         else:
#             model.load_state_dict(checkpoint, strict=False)
#             print("   ✅ Loaded raw structure weights (Fallback mode).")
            
#         model = model.to(DEVICE)
#         print("🚀 Recovery successful! Training will pick up right where it was paused.")
#     except Exception as e:
#         print(f"⚠️ Could not automatically map internal states: {e}. Defaulting to base parameters.")
#         model = model.to(DEVICE)
# else:
#     print("🏗️ No local checkpoint found. Loading clean pre-trained ResNet18 weights...")
#     model = model.to(DEVICE)
...
#         print(f"💾 Saved updated optimal checkpoint dictionary to {MODEL_SAVE_PATH}")
